# Hi there, I'm Kaizo Nakagawa
### Software Developer & Systems Engineer

I am a passionate developer focused on building efficient web applications, managing robust system infrastructures, and exploring the capabilities of embedded systems. Currently, I specialize in engineering digital solutions, optimizing server architectures, and bridging the gap between hardware and software.

---

### 🛠️ Tech Stack & Skills

#### **Web Development & Back-End**
![PHP](https://img.shields.io/badge/php-%23777BB4.svg?style=for-the-badge&logo=php&logoColor=white)
![Laravel](https://img.shields.io/badge/laravel-%23FF2D20.svg?style=for-the-badge&logo=laravel&logoColor=white)
![Vue.js](https://img.shields.io/badge/vue.js-%2335495e.svg?style=for-the-badge&logo=vuedotjs&logoColor=%234FC08D)
![JavaScript](https://img.shields.io/badge/javascript-%23F7DF1E.svg?style=for-the-badge&logo=javascript&logoColor=black)

#### **Systems & DevOps**
![Linux](https://img.shields.io/badge/Linux-FCC624?style=for-the-badge&logo=linux&logoColor=black)
![Arch Linux](https://img.shields.io/badge/Arch%20Linux-1793D1?style=for-the-badge&logo=arch-linux&logoColor=white)
![Debian](https://img.shields.io/badge/Debian-A81D33?style=for-the-badge&logo=debian&logoColor=white)
![Nginx](https://img.shields.io/badge/nginx-%23009639.svg?style=for-the-badge&logo=nginx&logoColor=white)

#### **Hardware & Embedded Systems**
![ESP32](https://img.shields.io/badge/ESP32-E7352C?style=for-the-badge&logo=espressif&logoColor=white)
![FreeRTOS](https://img.shields.io/badge/FreeRTOS-1A84C3?style=for-the-badge&logo=freertos&logoColor=white)
<br>
*C / C++ (ESP-NOW, HID Reporting, Mutex/Semaphore Architectures)*

#### **Tools & Environment**
* **Editors:** Cursor AI, Windsurf, VS Code
* **Database Management:** Navicat, MySQL, PostgreSQL
* **Version Control:** Git, Bitbucket, GitHub

---

### 🔭 Current Focus & Projects
* 🌐 **Enterprise Systems:** Building and optimizing large-scale KPI dashboards and industrial-grade web-based monitoring systems.
* 🐧 **Server Tuning:** Configuring secure Nginx architectures and robust directory permission management on Debian environments.
* ⚡ **Hardware Hacking:** Hard-coding Real-Time Operating Systems (RTOS) on ESP32 microcontrollers for low-latency wireless communication (ESP-NOW) and custom HID reporting.

---

### 📊 GitHub Statistics

|                                                                           My GitHub Stats                                                                            | Top Languages |
| :------------------------------------------------------------------------------------------------------------------------------------------------------------------: | :----------------------------------------------------------------------------------------------------------------------------------------------------------: |
| <img src="https://github-readme-stats.vercel.app/api?username=kaizo-nakagawa&show_icons=true&theme=tokyonight&count_private=true" alt="GitHub Stats" height="180px"> | <img src="https://github-readme-stats.vercel.app/api/top-langs/?username=kaizo-nakagawa&layout=compact&theme=tokyonight" alt="Top Languages" height="180px"> |

---

### 🌌 Beyond the Code
When I am away from my IDE, you can usually find me:
* 📸 Immersed in urban night exploration or capturing the minute details of nature through macro photography.
* 🗺️ Trekking through nature trails or riding out into the landscape to clear my mind.
* 🎮 Exploring the vast world of Teyvat (*Genshin Impact*) or surviving the depths of the Amazon (*Green Hell*).

---

<p align="center">
  <i>"Simplicity is the soul of efficiency."</i><br>
  💡 Always open to discussing Laravel architecture, Linux server optimization, or exciting new IoT projects!
</p>
